...
__global__ void matrixMultiplication(int p,int q,int r,int *d_A,int *d_B,int *d_E,int transpose){
    ...
    if(thIdx<p && thIdy<r){
        for(int i=0;i<q;i++){
			if(threadIdx.y==0)
				s_A[threadIdx.x] = d_A[blockIdx.x * blockDim.x * q + threadIdx.x*q + i];
			if(threadIdx.x==0){
				if(t==1){
                	s_B[threadIdx.y] = d_B[blockIdx.y * blockDim.y * q + i + threadIdx.y*q];
            	}
            	else{
                	s_B[threadIdx.y] = d_B[blockIdx.y * blockDim.y  + i*r + threadIdx.y];
            	}
			}
			
            temp +=  s_A[threadIdx.x]* s_B[threadIdx.y];
            __syncthreads();
        }
    }
	d_E[thIdx*r + thIdy] +=  temp;
}
...
void computE(int p, int q, int r, int *h_matrixA, int *h_matrixB, 
	...
	int grid_x,grid_y;
	grid_x = ceil(float(p) / 32);
	grid_y = ceil(float(r) / 32);
	dim3 grid3(grid_x,grid_y,1);
    dim3 block3(32,32,1);
	matrixMultiplication<<<grid3,block3>>>(p,q,r,d_matrixA,d_matrixB,d_matrixE,0);
    matrixMultiplication<<<grid3,block3>>>(p,q,r,d_matrixC,d_matrixD,d_matrixE,1);
